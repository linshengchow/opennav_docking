  ...
  const std::string & name, std::shared_ptr<tf2_ros::Buffer> tf)
{
  ...
  // Blind docking threshold
  nav2_util::declare_parameter_if_not_declared(
    node_, name + ".blind_docking_threshold", rclcpp::ParameterValue(0.3));
  ...
  node_->get_parameter(name + ".blind_docking_threshold", blind_docking_threshold_);
...
bool SimpleChargingDock::getRefinedPose(geometry_msgs::msg::PoseStamped & pose)
{
  ...
  if (!dock_pose_.header.frame_id.empty()){
    ...
    if (distance_to_dock < blind_docking_threshold_) {
